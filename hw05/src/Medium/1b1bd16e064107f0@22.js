...
function _output(FileAttachment) {
  return (
    FileAttachment("output.json").json()
  )
}

function _medium(d3, output, drag, invalidation) {
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 1200;
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("medium")).define("medium", ["d3", "output", "drag", "invalidation"], _medium);
